"""
Mini-Coding-Agent with Memory Manager
=====================================

Enhanced version of Mini-Coding-Agent with Memory Manager optimization.
...
HELP_TEXT = "/help, /memory, /session, /reset, /exit, /stats"
WELCOME_ART = (
    "/\\     /\\",
    "{  `---'  }",
    "{  O   O  }",
...
HELP_DETAILS = "\n".join(
    [
        ...
        "/stats   Show memory manager statistics.",
        "/exit    Exit the agent.",
    ]
)
...
class WorkspaceContext:
    ...
    def build(cls, cwd):
        ...
        return cls(
            ...
            default_branch=(git(["symbolic-ref", "--short", "refs/remotes/origin/HEAD"], "origin/main") or "origin/main").replace("origin/", "", 1),
...
class MiniAgent:
    def __init__(
        ...
        self.memory_config = memory_config or DEFAULT_MEMORY_MANAGER_CONFIG
    ...
    def from_session(cls, model_client, workspace, session_store, session_id, **kwargs):
        ...
        if "memory_manager" in session:
            try:
                agent.memory_manager.deserialize(session["memory_manager"])
            except Exception:
                pass
    ...
    def memory_text(self):
        ...
        enhanced_memory = self.memory_manager.build_memory_text()
    ...
    def _save_session(self):
        self.session["memory_manager"] = self.memory_manager.serialize()
        self.session_path = self.session_store.save(self.session)
    ...
    def ask(self, user_message):
        ...
        self.memory_manager.process_user_message(user_message)
        ...
        while tool_steps < self.max_steps and attempts < max_attempts:
            ...
            if kind == "tool":
                ...
                compressed_result = self.memory_manager.process_tool_result(name, args, "")
                ...
                tool_calls = [self.session["history"][-1]]
                self.memory_manager.process_round(user_message, assistant_response, tool_calls)
            ...
            tool_calls = [item for item in self.session["history"] if item["role"] == "tool"]
            self.memory_manager.process_round(user_message, final, tool_calls)

            self.remember(memory["notes"], clip(final, 220), 5)

            if self._check_and_compact():
                pass
    ...
    def _check_and_compact(self):
        ...
        should_compact, reason = self.memory_manager.should_compact(history, estimated_tokens)

        if should_compact:
            compressed_history = self.memory_manager.compact_history(history)
            self.session["history"] = compressed_history
            self._save_session()
            return True
        return False

    def _estimate_history_tokens(self):
        """估算历史 token 数"""
        total_chars = 0
        for item in self.session["history"]:
            content = str(item.get("content", ""))
            total_chars += len(content)
        chinese_chars = sum(1 for c in str(total_chars) if '\u4e00' <= c <= '\u9fff')
    ...
    def run_tool(self, name, args):
        ...
        try:
            ...
            compressed_result = self.memory_manager.process_tool_result(name, args, result)
    ...
    def stats(self):
        ...
        lines = ["[Memory Manager Stats]", ""]
        lines.append(f"总节省 Token 数: {stats['total_tokens_saved']}")
        lines.append(f"工具输出压缩比: {stats['compression_ratio']:.2%}")
        lines.append(f"约束保留数: {stats['constraints_count']}")
        lines.append(f"摘要历史数: {stats['summary_history_count']}")
        lines.append(f"任务注册数: {stats['task_registry_count']}")
        lines.append(f"当前阶段: {stats['current_phase']}")
        return "\n".join(lines)
...
def build_welcome(agent, model, host):
    ...
    rows.extend(
        [
            ...
            center("with Memory Manager v2.1"),
            divider("-"),
            row(""),
...
def build_arg_parser():
    parser = argparse.ArgumentParser(
        ...
        description="Minimal coding agent for Ollama models with Memory Manager.",
    )
